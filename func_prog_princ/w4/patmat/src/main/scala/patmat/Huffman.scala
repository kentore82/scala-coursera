...
object Huffman {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case left :: right :: cs => (makeCodeTree(left, right) :: cs)
      .sortWith((left, right) => weight(left) < weight(right))
    case _ => trees
  }
  ...
  /**
    ...
    */
  def until(isTrue: List[CodeTree] => Boolean, tryAgain: List[CodeTree] => List[CodeTree])
           (trees: List[CodeTree]): List[CodeTree] = {
    if (isTrue(trees)) trees
    else until(isTrue, tryAgain)(tryAgain(trees))
  }
...
object Main extends App {
  ...
  val myTree = Huffman.frenchCode
  val myText = decode(myTree,Huffman.secret)
